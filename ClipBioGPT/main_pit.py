...
def train(train_dataloader, model, criterion, optimizer, epoch, device):
    ...
    for i, (_, v_f, q, labels) in enumerate(train_dataloader, 0):
        # text features
        input_ids = torch.squeeze(q['input_ids'], 1).to(device)
        attention_mask = torch.squeeze(q['attention_mask'], 1).to(device)

        # Visual features
        visual_features = v_f
        visual_features['pixel_values'] = torch.squeeze(visual_features['pixel_values'], 1).to(device)
...
if __name__ == '__main__':
    ...
    model = ClipBioGPT(num_class=args.num_class)
