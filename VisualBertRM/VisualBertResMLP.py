...
class ResMLP_BLocks(nn.Module): 
    def __init__(self, config, token_size): 
        super().__init__() 
        self.linear_patches = nn.Linear(token_size, token_size)
        self.layerNorm1 = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.mlp_channels = Mlp(config)
...
class VisualBertResMLPModel(VisualBertPreTrainedModel):
    ...
    def __init__(self, config, token_size=26, add_pooling_layer=True):
        ...
        if self.bypass_transformer:
            # self.additional_layer = VisualBertLayer(config)  # VisualBertLayer not found
            self.additional_layer = VisualBertResMLPLayer(config)
