...
from model import VisualBertResMLP
...
def get_arg():
    ...
    parser.add_argument('--checkpoint_dir', default='checkpoints/pit/lr2e-5_', help='path to store weights')
...
if __name__ == '__main__':
    ...
    # dataloader
    train_dataset = PitVis24VQA(train_seq, folder_head, folder_tail, patch_size=4)
    train_dataloader = DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=8)
    val_dataset = PitVis24VQA(val_seq, folder_head, folder_tail, patch_size=4)
    ...
    model = VisualBertResMLP(vocab_size=len(tokenizer), layers=6, n_heads=8, num_class=args.num_class, token_size=41)
